...
class CNN(nn.Module):
    def __init__(self, kernel_size, stride, padding): # kernel = 3, stride = 2, padding = 1
        ...
        self.conv2 = nn.Conv2d(in_channels = 16, out_channels = 64, kernel_size = kernel_size, stride = stride, padding = padding, bias = False)
...
class HMHSA(nn.Module):
    def __init__(self, dim, head, grid_size, ds_ratio, drop): # we have an embedding and we are going to split it in (heads) =! parts
        ...
        self.qkv = nn.Linear(self.dim, self.dim, bias = False) # q, k, v are all the same 
        #change to this self.qkv = nn.Conv2d(dim, dim * 3, 1)
        # put bias = False
